...
struct struct_name ## List { \
	 ...
	 * The bigger, the more we will avoid calling realloc for each new inserted struct, improving its performance. \
...
void initialize_##struct_name_snake##_list(struct struct_name##List *list) { \
	list->page_array_granularity = initial_page_array_granularity; \
	list->short_item_name##s_per_page = initial_items_per_page; \
...
\
			list->sorted_##short_item_name##s = realloc(list->sorted_##short_item_name##s, new_page_array_length * list->short_item_name##s_per_page * sizeof(struct struct_name *)); \
...
\
		list->page_array[list->short_item_name##_count / list->short_item_name##s_per_page] = new_page; \
	} \
\
