...
int merge_stacks(struct Stack *stack, const struct Stack *other_stack) {
	...
	for (i = 0; i < required_bytes; i++) {
		...
		const int this_defined = stack->defined_and_merged[this_index / 4 / sizeof(packed_data_t)] & 1 << (this_index % (4 * sizeof(packed_data_t))) * 2;
		const int other_defined = other_stack->defined_and_merged[other_index / 4 / sizeof(packed_data_t)] & 1 << (other_index % (4 * sizeof(packed_data_t))) * 2;
		const int this_merged = stack->defined_and_merged[this_index / 4 / sizeof(packed_data_t)] & 2 << (this_index % (4 * sizeof(packed_data_t))) * 2;
		const int other_merged = other_stack->defined_and_merged[other_index / 4 / sizeof(packed_data_t)] & 2 << (other_index % (4 * sizeof(packed_data_t))) * 2;
		int is_defined;
...
int changes_on_merging_stacks(const struct Stack *stack, const struct Stack *other_stack) {
	...
	for (i = 0; i < required_bytes; i++) {
		...
		if (is_defined) {
			...
			if ((stack->defined_and_merged[this_index / 4 / sizeof(packed_data_t)] & 3 << (this_index % (4 * sizeof(packed_data_t))) * 2) != 2 << (this_index % (4 * sizeof(packed_data_t))) * 2) {
				return 1;
			}
		}
		else {
			if (stack->defined_and_merged[this_index / 4 / sizeof(packed_data_t)] & 3 << (this_index % (4 * sizeof(packed_data_t))) * 2) {
				return 1;
			}
		}

		if (i & 1) {
			const packed_data_t relative = stack->relative[this_index / 16 / sizeof(packed_data_t)];
			const packed_data_t mask = 1 << this_index % (16 * sizeof(packed_data_t));
			if (this_relative && other_relative) {
				if ((relative & mask) == 0) {
					return 1;
				}
			}
			else if (relative & mask) {
				return 1;
			}
		}
	}
